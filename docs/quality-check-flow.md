...
| `MAX_FEEDBACK_LOOPS` | `2` | 最多执行 2 轮质检。由于首次质检已经计为第 1 轮，因此当前最多只会发生 1 次回退。 |
...
质检 Agent 并不会看到完整的任务状态，而是看到代码从报告、分析、证据和来源中截取出的摘要。

| Prompt 区块 | Agent 实际能看到的信息 | 截断或数量限制 |
|---|---|---|
| 报告内容 | `report.markdown_content` | 只取前 `4000` 个字符。 |
| 分析摘要 | 每个竞品的名称、定位、定价摘要、关联证据数量 | 定位只取前 `100` 字符；定价只取前 `80` 字符；会遍历全部分析。 |
...
```text
- 竞品={competitor_name}；
  定位={positioning 前 100 字符}；
  定价={pricing_summary 前 80 字符}；
...
| `retry_instructions` | `string \| null` | 面向人类阅读的整体改进指导。 | `retry_collection` 时可转为 `qa_retry_guidance`；也会写入质检结果表。当前直接 `retry_analysis` 不会使用它。 |
| `retry_queries` | `array` | 需要重新采集时使用的精确搜索 query。 | 仅当最终决策为 `retry_collection` 时复制到 `AgentState.qa_retry_queries`。 |
| `issues` | `array` | 发现的问题列表。 | 用于推断回退类型、定位需要重分析的竞品，并展示在前端。 |
...
| `competitor_name` | `string` | 相关竞品名称，或 `report`、`system`。 | 用于定位需要重新分析的竞品。 |
| `description` | `string` | 问题描述。 | 落库并展示给用户。 |
| `fix_suggestion` | `string` | 修复建议。 | 当 Agent 未返回 `retry_instructions` 时，可用于拼接重新采集指导。 |
...
当前 `MAX_FEEDBACK_LOOPS = 2`，因此第 2 轮会先命中“达到最大轮次并强制通过”。在当前配置下，“分数未提高时强制通过”的分支实际不会在第 2 轮触发。
...
| `retry_collection` | `qa_retry_guidance` | 优先使用 Agent 的 `retry_instructions`；若为空，则拼接采集类 issue 的“竞品名: fix_suggestion”。 |
...
| 采集轮次标记 | `collection_iteration = 0`。 | 第一次回退时 `collection_iteration = 1`。 |
...
- 将 `qa_retry_guidance` 注入分析 Agent Prompt。
- 生成新的分析 ID。
...
`retry_collection` 会设置 `qa_retry_guidance`，因此重新采集后的结构化分析会收到质检改进指导。

但是，当前直接 `retry_analysis` 分支只设置 `qa_retry_analysis_ids`，没有设置 `qa_retry_guidance`。因此直接回退到 `structured_analysis` 时：
...
| 是否注入质检指导 | 否 | 是，使用 `qa_retry_guidance` | 当前实现不注入 |
...
新报告生成后再次进入 `quality_check`。由于当前 `MAX_FEEDBACK_LOOPS = 2`，第 2 轮质检无论分数和问题如何，都会被系统改为 `pass`。
...
1. 当前最多只会发生一次回退。第 2 轮质检会无条件强制通过。
...
4. `retry_collection` 如果没有有效 `retry_queries`，资料采集节点会因为 `qa_retry_queries` 为空而回到常规模板规划，而不是精确补采。
   - 此时后续结构化分析也无法从 `retry_queries` 定位受影响竞品，并且不会注入 `qa_retry_guidance`。
5. `retry_queries[].competitor_name` 无法匹配已选竞品时，对应 query 会被忽略。
6. 质检 Agent 只看到截断后的报告、证据和来源摘要，因此它对报告后半部分、完整引用 URL 和完整证据原文的判断能力有限。
