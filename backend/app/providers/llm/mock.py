...
class MockLLMProvider(LLMProvider):
    ...
    def understand_requirement(self, user_requirement: str) -> dict[str, Any]:
        ...
        if any(
            ...
            keyword in text
            for keyword in ["会议", "纪要", "otter", "fireflies", "转写"]
        ):
            input_type = "existing_product"
            ...
            keyword in text
            for keyword in ["竞品分析", "市场调研", "competitive analysis"]
        ):
            input_type = "product_idea"
            target_product = None
            product_description = user_requirement
            ...
            target_product = (
                "水壶产品"
                if any(keyword in text for keyword in ["水壶", "水瓶", "bottle"])
                else "智能保温杯"
            )
            domain = "水壶/饮具"
    ...
    def extract_focus_profile(
        ...
        return {
            ...
            if explicit_focuses
                ...
                "如果用户未补充侧重点，报告将按功能、价格、评价和市场定位进行均衡分析。"
            ],
        }
    ...
    def understand_target(
        ...
        name = (
            ...
            or requirement.get("product_description")
            or requirement.get("domain", "目标对象")
        )
        ...
        text = "\n".join(
            ...
            for item in target_search_results
        )
        if "企业协作" in domain or "办公" in domain:
            ...
            positioning = (
                f"{name} 面向开发者提供 AI 辅助编码、代码理解和研发效率提升能力。"
            )
            ...
            keyword in domain for keyword in ["保温杯", "水杯", "水壶", "水瓶", "饮具"]
        ):
            capabilities = ["保温保冷", "便携设计", "材质安全", "容量规格", "饮水体验"]
            use_cases = ["办公室饮水", "通勤携带", "户外运动", "健康管理"]
            positioning = (
                f"{name} 面向日常饮水、通勤和户外场景，解决便携、保温和饮水体验需求。"
            )
        else:
            capabilities = requirement.get("core_capabilities") or [
                "核心流程自动化",
                "信息整理",
                "报告生成",
            ]
            ...
            positioning = (
                f"{name} 属于“{domain}”方向，具体定位需结合公开资料进一步确认。"
            )
        return {
            "name": str(name),
            "category": domain,
    ...
    def extract_competitors(
        ...
        target_category = target_understanding.get("category") or requirement.get(
            "domain", ""
        )
        ...
        if (
            "企业协作" in target_category
            or "办公" in target_category
            or "钉钉" in target_name_lower
        ):
            ...
            keyword in target_category
            for keyword in ["保温杯", "水杯", "水壶", "水瓶", "饮具"]
        ):
            global_names = [
                "Stanley Quencher",
                "Hydro Flask",
            ...
            global_names = (
                names[:4]
                if len(names) >= 4
                else (names + ["Slack", "Microsoft Teams"])[:4]
            )
            china_names = (
                names[4:8] if len(names) >= 8 else names[4:] + ["钉钉", "企业微信"]
            )[:4]
            global_websites = {}
            china_websites = {}
            ...
            competitors.append(
                {
                    ...
                    "description": _describe_candidate(
                        name, related_result, requirement
                    ),
                    ...
                    "source_ids": [related_result.get("url")]
                    if related_result.get("url")
                    else [],
                    "evidence_ids": [],
    ...
    def analyze_competitor(
        ...
        evidence_ids = [
            ...
            or item.get("related_dimension", "evidence")
    ...
    def generate_report(
        ...
        for analysis in analyses:
            ...
            dynamic_claims = _dynamic_bundle_claims(
                bundle_by_competitor.get(analysis.get("competitor_id"))
            )
            lines.extend(
                [
                    f"## {competitor_name}",
            ...
            if dynamic_claims:
                ...
                for claim in dynamic_claims:
                    ...
                    lines.append(
                        f"- {claim.get('label')}: {claim.get('text')}{citation}"
                    )
            lines.append("")
        lines.extend(
            [
    ...
    def qa_check_report(
        ...
        for issue in issues:
            ...
            dimension_scores[dimension] = max(
                0.3, dimension_scores[dimension] - penalty
            )

        retry_instructions = (
            "; ".join(i["fix_suggestion"] for i in issues if i.get("fix_suggestion"))
            or None
        )
                ...
                retry_queries.append(
                    {
                        ...
                        "slot": "core_features",
                        "query": f"{comp} core features capabilities detailed",
                    }
                )
                retry_queries.append(
                    {
                        "competitor_name": comp,
                        "slot": "pricing",
                        "query": f"{comp} pricing plans detailed cost",
                    }
                )
                ...
                retry_queries.append(
                    {
                        "competitor_name": comp,
                        "slot": "pricing",
                        "query": f"{comp} pricing plans tiers comparison",
                    }
                )
    ...
    def qa_verify_issues(
        ...
        for item in evidence:
            ...
            for comp_name, comp_id in competitor_id_by_name.items():
                if comp_id and cid == comp_id:
                    evidence_count_by_name[comp_name] = (
        ...
        return {
            ...
            "retry_instructions": "; ".join(
                ...
                for issue in open_issues
                if issue.get("id") in {item["issue_id"] for item in unresolved}
            )
            or None,
        }
    ...
    def classify_chat_intent(
        ...
        is_redo = any(kw in text for kw in redo_keywords)
        return {
            "intent": "report_redo" if is_redo else "report_edit",
            "reason": "检测到重新调研关键词" if is_redo else "默认视为报告修改",
        }
        ...
        if "删除" in user_message or "去掉" in user_message:
            for target in [
                ...
                if target in user_message:
                    ...
                    report_markdown = _re.sub(
                        rf"\n##\s*.*?{target}.*?\n(?:\n|.)*?(?=\n## |\Z)",
                        "",
                        report_markdown,
                        flags=_re.IGNORECASE,
                    )
                    return report_markdown.strip() + note
        if "增加" in user_message or "添加" in user_message or "补充" in user_message:
            topic = (
                user_message.replace("增加", "")
                .replace("添加", "")
                .replace("补充", "")
                .strip()
            )
            section = f"\n## 补充分析：{topic}\n\n根据用户反馈，针对 **{topic}** 进行补充分析。基于现有竞品数据和证据，相关要点如下：\n\n- 各竞品在 **{topic}** 维度表现各异，需结合具体场景评估\n- 建议进一步收集专项数据以完善该维度的分析\n"
            return report_markdown.strip() + section + note
        return report_markdown + note
    ...
    def generate_chat_queries(
        ...
        competitors = [c for c in existing_competitors if c]
        queries = []
            ...
            queries.append(
                {
                    "competitor_name": comp,
                    "slot": "user_feedback",
                    "query": f"{comp} 用户评价 {user_message[:40]}",
                }
            )
        return {
            "retry_queries": queries,
            "retry_instructions": f"用户反馈：{user_message}。请根据新的搜索查询补充资料，重新分析后生成报告。",
    ...
    def generate_revision_search_plan(
        ...
        return {
            "search_plan": [
                {
                    "competitor_name": target.get("name", "目标产品"),
                    "purpose": "核实用户反馈中指出的问题",
                    "queries": [
                        f"{target.get('name', '目标产品')} {user_message[:40]}",
                        f"{target.get('name', '目标产品')} product positioning review",
                    ],
                    "expected_evidence": "官网定位、功能介绍、第三方评价",
                }
                for target in targets
            ],
            "plan_summary": "围绕用户反馈补充检索资料",
        }
    ...
    def generate_revision_plan(
        ...
        return {
            "revision_type": "section_rewrite"
            if intent_result.get("need_search")
            else "minor",
            "structure_change_needed": bool(intent_result.get("need_search")),
            "structure_change_reason": "根据用户反馈需要核实并调整相关结论"
            if intent_result.get("need_search")
            else "仅需局部调整表达",
            "sections_to_change": [
                {
                    ...
                    "change_type": "rewrite",
                    "reason": "响应用户反馈",
                    "new_direction": user_message,
                }
                for section in (intent_result.get("affected_sections") or ["报告正文"])
            ],
    ...
    def generate_revision_summary(
        ...
        return f"已根据你的反馈更新报告，重点调整：{user_message[:60]}。"


def _extract_product_names(
    search_results: list[dict[str, Any]], target_name: str
) -> list[str]:
    names = []
    for result in search_results:
        ...
        for name in found:
            if (
                ...
                and not _looks_mock_generic_name(name)
            ):
                names.append(name)
    seen = set()
    unique = []
...
def _looks_mock_generic_name(name: str) -> bool:
    ...
    if stripped.startswith(("我想", "我要", "想做", "要做")):
        return True
    return any(
        term in stripped for term in ["同类产品榜单", "竞品对比", "替代方案讨论"]
    )


def _find_related_result(
    name: str, search_results: list[dict[str, Any]]
) -> dict[str, Any]:
    for result in search_results:
...
def _describe_candidate(
    name: str, related_result: dict[str, Any], requirement: dict[str, Any]
) -> str:
...
def _dynamic_bundle_claims(bundle_item: object) -> list[dict[str, Any]]:
    ...
    return [
        ...
        and str(claim.get("claim_type", "")).startswith("focus:")
    ]
...
def _mock_focus_analysis(
    focus_schema: object, evidence: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    for focus in focus_schema[:6]:
        ...
        matched = [
            item
            for item in evidence
            if label in str(item.get("related_dimension", ""))
            or label in str(item.get("summary", ""))
        ]
        if not matched:
            matched = evidence[:2]
        evidence_ids = [item.get("id") for item in matched if item.get("id")]
        verdict = (
            f"围绕“{label}”，现有证据显示该产品已有相关信息，但仍需结合真实来源复核。"
        )
        if not evidence_ids:
            verdict = "证据中未涉及"
        results.append(
...
def _should_mock_ask_clarification(
    user_requirement: str, requirement: dict[str, Any]
) -> bool:
    ...
    if any(
        ...
        return False
    domain = str(
        requirement.get("domain") or requirement.get("possible_market_category") or ""
    )
    return any(
        keyword in domain
        for keyword in [
    ...
    ) or any(keyword in text for keyword in ["笔记", "文档", "工具", "软件", "竞品"])
